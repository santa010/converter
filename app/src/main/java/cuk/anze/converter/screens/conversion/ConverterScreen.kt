...
class ConverterScreen: AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        disposable = Observable.interval(0, 1, TimeUnit.SECONDS)
            .flatMap { conversionService.getLatestConversionRates("EUR") }
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe { response ->
                response?.rates?.let { rates ->
                    if (rv_currencyList.scrollState == RecyclerView.SCROLL_STATE_IDLE) {
                        adapter.updateCurencyValue(CurrencyInfo("EUR", getImageUrlForTicker("EUR"), currencyNameMap["EUR"] ?: "not provided", 1.0))
                        adapter.updateCurencyValues(
                            rates.map {
                                CurrencyInfo(
                                    it.key,
